"""BRIDGE Model Training Utilities with TLSQL

This module provides utilities for training BRIDGE models using TLSQL statements.
...
from tlsql.examples.bridge.utils.bridge_pipeline import prepare_bridge_data
...
def train_bridge_model(model, target_table, non_table_embeddings, adj, epochs, lr, wd, k_folds=None):
    """Train BRIDGE model with optional k-fold cross-validation
    ...
    Args:
        ...
        lr: Learning rate
        wd: Weight decay
        k_folds: Number of folds for cross-validation. If None or <= 1, uses fixed train/val/test masks.
                 If > 1, performs k-fold cross-validation.

    Returns:
        If k_folds is None or <= 1: (model, best_val_acc, test_acc)
        If k_folds > 1: List of fold results, each containing:
            {'fold': int, 'best_val_acc': float, 'test_acc': float, 'mean_time': float, 'total_time': float}
    """
    if k_folds is None or k_folds <= 1:
        # Standard training with fixed masks
        y = target_table.y
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(kfold.split(remaining_indices), 1):
            ...
            for epoch in range(1, epochs + 1):
                start = time.time()
                train_loss = train(
                    fold_model, optimizer, target_table, non_table_embeddings, adj, y, fold_train_mask
                )
                train_acc, val_acc, tmp_test_acc = test(
...
def _load_train_data_dict(executor, train_ast, sql_generator, target_table_name,
    ...
    and load related tables based on database name.
    """
    if train_ast:
    ...
    # Determine related tables based on database name
    database_name = executor.config.database.lower()

    # Known dataset configurations
    dataset_configs = {
        'tml1m': {
            'relation_tables': ['ratings'],
            'aux_tables': ['movies']
        },
        'tacm12k': {
            'relation_tables': ['citations'],
            'aux_tables': []
        }
    }
    ...
    config_tables = None
    for db_key, config in dataset_configs.items():
        if db_key in database_name:
    ...
    if config_tables:
        ...
        # Fallback: load all other tables
...
def _parse_tlsql_statements(train_sql, validate_sql, predict_sql):
    """Parse all TLSQL statements and extract key information
...
def prepare_data_from_tlsql(
    ...
    Level III: VALIDATE WITH (if not specified, perform k=5 fold cross-validation)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Get data using tlsql")
